...
if (isset($_GET['confirm']) && $_GET['confirm'] === 'yes' && isset($_GET['user'])) {
    ...
    try {
        // Delete from faculty
        $stmt1 = $conn->prepare("DELETE FROM faculty WHERE username = ?");
        ...
        $conn->rollback();
        echo "<script>alert('Failed to delete account.'); window.location.href = 'home.html';</script>";
    }
    exit;
}
        ...
        if ($password !== $row['password']) {
            ...
            echo "<script>
                if (confirm('Are you sure you want to delete your account?')) {
                    window.location.href = 'deletefacpfp.php?confirm=yes&user=" . urlencode($username) . "';
                } else {
...
<body>
    <div class="main-content">
        ...
        <div class="upload-card">
            ...
            <form action="deletefacpfp.php" method="post">
                ...
                <input type="submit" value="Delete Faculty" class="sb">
            </form>
        </div>
    </div>
    <footer>
