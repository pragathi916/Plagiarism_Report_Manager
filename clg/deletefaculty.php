...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_delete_faculty = "DELETE FROM faculty WHERE username=?";
    $stmt_delete_faculty = $conn->prepare($sql_delete_faculty);
    $stmt_delete_faculty->bind_param("s", $delete_username);
    $stmt_delete_faculty->execute();
    if ($stmt_delete_faculty->affected_rows > 0) {
        // If deleted from faculty table, delete from users table if exists
        ...
        echo "<script>alert('Faculty deleted successfully.'); window.location.href='admin.html';</script>";
    } else {
        echo "<script>alert('Faculty not found.'); window.history.back();</script>";
    }
